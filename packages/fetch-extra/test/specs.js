...
export default host => {
	...
	describe('client.set', function () {
		...
		it('throw error is client.set() with invalid key', function () {
			assert.throws(() => request().set());
		});

		it('client.set() with invalid function type', function () {
	...
	describe('url', function () {
		...
		it('url string equals "/"', async function () {
			const url = '/';
			const client = request({ url });
			const composed = await client.compose();
			assert(composed.url === url);
	...
	describe('custom qs library', function () {
		it('queryStringify', async function () {
			const client = request({
				url: '/',
				method: 'POST',
				body: { hello: 'world' },
			...
			const composed = await client.compose();
			assert(composed.body === 'foo=bar');
		});

		it('queryParse', async function () {
			const client = request({
				url: '/',
				query: { hello: 'world' },
